...
// _D3XP : rename all gameLocal.time to GAMELOCAL_SLOW_TIME for merge!
		...
		if ( ent ) {
			for ( int j = 0; j < MAX_RENDERENTITY_GUI; j++ ) {
				if ( ent->GetRenderEntity() && ent->GetRenderEntity()->gui[ j ] ) {
					...
					ent->GetRenderEntity()->gui[ j ]->StateChanged( GAMELOCAL_SLOW_TIME, true );
				}
			}
			ent->UpdateVisuals();
		}
...
void idMover::SetGuiStates( const char *state ) {
	...
	for ( i = 0; i < MAX_RENDERENTITY_GUI; i++ ) {
		if ( renderEntity.gui[ i ] ) {
			...
			renderEntity.gui[ i ]->StateChanged( GAMELOCAL_SLOW_TIME, true );
		}
	}
}
...
void idMover::Event_UpdateMove( void ) {
	...
	switch( move.stage ) {
		case ACCELERATION_STAGE: {
			physicsObj.SetLinearExtrapolation( EXTRAPOLATION_ACCELLINEAR, GAMELOCAL_SLOW_TIME, move.acceleration, org, move.dir, vec3_origin );
			if ( move.movetime > 0 ) {
		...
		case LINEAR_STAGE: {
			physicsObj.SetLinearExtrapolation( EXTRAPOLATION_LINEAR, GAMELOCAL_SLOW_TIME, move.movetime, org, move.dir, vec3_origin );
		...
		case DECELERATION_STAGE: {
			physicsObj.SetLinearExtrapolation( EXTRAPOLATION_DECELLINEAR, GAMELOCAL_SLOW_TIME, move.deceleration, org, move.dir, vec3_origin );
			move.stage = FINISHED_STAGE;
			break;
		}
		case FINISHED_STAGE: {
			if ( g_debugMover.GetBool() ) {
				gameLocal.Printf( "%d: '%s' move done\n", GAMELOCAL_SLOW_TIME, name.c_str() );
			}
			DoneMoving();
			break;
		}
...
void idMover::Event_UpdateRotation( void ) {
	...
	switch( rot.stage ) {
		case ACCELERATION_STAGE: {
			physicsObj.SetAngularExtrapolation( EXTRAPOLATION_ACCELLINEAR, GAMELOCAL_SLOW_TIME, rot.acceleration, ang, rot.rot, ang_zero );
			if ( rot.movetime > 0 ) {
		...
		case LINEAR_STAGE: {
			if ( !stopRotation && !rot.deceleration ) {
				physicsObj.SetAngularExtrapolation( extrapolation_t(EXTRAPOLATION_LINEAR|EXTRAPOLATION_NOSTOP), GAMELOCAL_SLOW_TIME, rot.movetime, ang, rot.rot, ang_zero );
			} else {
				physicsObj.SetAngularExtrapolation( EXTRAPOLATION_LINEAR, GAMELOCAL_SLOW_TIME, rot.movetime, ang, rot.rot, ang_zero );
			}
		...
		case DECELERATION_STAGE: {
			physicsObj.SetAngularExtrapolation( EXTRAPOLATION_DECELLINEAR, GAMELOCAL_SLOW_TIME, rot.deceleration, ang, rot.rot, ang_zero );
			rot.stage = FINISHED_STAGE;
			break;
		}
		case FINISHED_STAGE: {
			...
			if ( stopRotation ) {
				...
				physicsObj.SetAngularExtrapolation( extrapolation_t(EXTRAPOLATION_LINEAR|EXTRAPOLATION_NOSTOP), GAMELOCAL_SLOW_TIME, 0, ang, rot.rot, ang_zero );
			}
...
void idMover::Event_TeamBlocked( idEntity *blockedEntity, idEntity *blockingEntity ) {
	if ( g_debugMover.GetBool() ) {
		gameLocal.Printf( "%d: '%s' stopped due to team member '%s' blocked by '%s'\n", GAMELOCAL_SLOW_TIME, name.c_str(), blockedEntity->name.c_str(), blockingEntity->name.c_str() );
	}
}
...
void idMover::Event_PartBlocked( idEntity *blockingEntity ) {
	...
	if ( g_debugMover.GetBool() ) {
		gameLocal.Printf( "%d: '%s' blocked by '%s'\n", GAMELOCAL_SLOW_TIME, name.c_str(), blockingEntity->name.c_str() );
	}
}
...
void idMover::Event_MoveAccelerateTo( float speed, float time ) {
	...
	StartSound( "snd_accel", SND_CHANNEL_BODY2, 0, false, NULL );
	StartSound( "snd_move", SND_CHANNEL_BODY, 0, false, NULL );
	physicsObj.SetLinearExtrapolation( EXTRAPOLATION_ACCELLINEAR, GAMELOCAL_SLOW_TIME, move.acceleration, org, dir * ( speed - v ), dir * v );
}
...
void idMover::Event_MoveDecelerateTo( float speed, float time ) {
	...
	StartSound( "snd_decel", SND_CHANNEL_BODY2, 0, false, NULL );
	StartSound( "snd_move", SND_CHANNEL_BODY, 0, false, NULL );
	physicsObj.SetLinearExtrapolation( EXTRAPOLATION_DECELLINEAR, GAMELOCAL_SLOW_TIME, move.deceleration, org, dir * ( v - speed ), dir * speed );
}
...
void idMover::Event_StartSpline( idEntity *splineEntity ) {
	...
	spline->ShiftTime( GAMELOCAL_SLOW_TIME - spline->GetTime( 0 ) );
...
void idElevator::Event_Touch( idEntity *other, trace_t *trace ) {
	...
	if ( GAMELOCAL_SLOW_TIME < lastTouchTime + 2000 ) {
		return;
	}
	...
	lastTouchTime = GAMELOCAL_SLOW_TIME;
...
void idMover_Binary::Event_ReturnToPos1( void ) {
	MatchActivateTeam( MOVER_2TO1, GAMELOCAL_SLOW_TIME );
}
...
void idMover_Binary::Event_Reached_BinaryMover( void ) {
	...
	if ( moverState == MOVER_1TO2 ) {
		...
		SetMoverState( MOVER_POS2, GAMELOCAL_SLOW_TIME );
		...
		SetMoverState( MOVER_POS1, GAMELOCAL_SLOW_TIME );
...
void idMover_Binary::GotoPosition2( void ) {
	...
	if ( moverState == MOVER_POS1 ) {
		MatchActivateTeam( MOVER_1TO2, GAMELOCAL_SLOW_TIME );
...
void idMover_Binary::Use_BinaryMover( idEntity *activator ) {
	...
	if ( moverState == MOVER_POS1 ) {
		...
		MatchActivateTeam( MOVER_1TO2, GAMELOCAL_SLOW_TIME + USERCMD_MSEC );
...
void idDoor::Spawn( void ) {
	...
	if ( moveMaster == this ) {
		...
		if ( noTouch || health ) {
			...
			PostEventMS( &EV_Mover_MatchTeam, 0, moverState, GAMELOCAL_SLOW_TIME );
...
void idDoor::Event_SpawnDoorTrigger( void ) {
	...
	MatchActivateTeam( moverState, GAMELOCAL_SLOW_TIME );
}
...
void idDoor::Event_Touch( idEntity *other, trace_t *trace ) {
	...
	if ( trigger && trace->c.id == trigger->GetId() ) {
		...
		if ( other && other->IsType( idPlayer::Type ) && IsLocked() && GAMELOCAL_SLOW_TIME > nextSndTriggerTime ) {
			StartSound( "snd_locked", SND_CHANNEL_ANY, 0, false, NULL );
			nextSndTriggerTime = GAMELOCAL_SLOW_TIME + 10000;
		}
	}
}
...
void idDoor::Event_SpectatorTouch( idEntity *other, trace_t *trace ) {
	...
	if ( p->lastSpectateTeleport > GAMELOCAL_SLOW_TIME - 1000 ) {
		return;
	}
	if ( trigger && !IsOpen() ) {
		...
		p->lastSpectateTeleport = GAMELOCAL_SLOW_TIME;
	}
}
...
void idRotater::Spawn( void ) {
	...
	physicsObj.SetLinearExtrapolation( EXTRAPOLATION_NONE, GAMELOCAL_SLOW_TIME, 0, GetPhysics()->GetOrigin(), vec3_origin, vec3_origin );
	physicsObj.SetAngularExtrapolation( extrapolation_t(EXTRAPOLATION_LINEAR|EXTRAPOLATION_NOSTOP), GAMELOCAL_SLOW_TIME, 0, GetPhysics()->GetAxis().ToAngles(), ang_zero, ang_zero );
	SetPhysics( &physicsObj );
...
void idRotater::Event_Activate( idEntity *activator ) {
	...
	physicsObj.SetAngularExtrapolation( extrapolation_t(EXTRAPOLATION_LINEAR|EXTRAPOLATION_NOSTOP), GAMELOCAL_SLOW_TIME, 0, physicsObj.GetAxis().ToAngles(), delta, ang_zero );
}
...
void idRiser::Event_Activate( idEntity *activator ) {
	...
	if ( !IsHidden() && spawnArgs.GetBool("hide")  ) {
		...
		physicsObj.SetLinearExtrapolation( EXTRAPOLATION_LINEAR, GAMELOCAL_SLOW_TIME, time * 1000, physicsObj.GetOrigin(), delta, vec3_origin );
	}
}
